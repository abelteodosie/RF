...
public class FileUtils 
{
	...
	private static Map<String, Integer> getDistincElementsCounterMap(List <String>clase)
	{
		Map<String, Integer> counterMap = new HashMap<String, Integer>();
		for (int j = 0; j < clase.size(); j++) {
			if (counterMap.containsKey(clase.get(j))) {
				int count = counterMap.get(clase.get(j));
				counterMap.put((clase.get(j)), ++count);
			} else {
				counterMap.put((clase.get(j)), 1);
			}
		}
		return counterMap;
	}

	protected static String calculateKNN(List<String>clasa)
	{
		Map <String, Integer> counterMap = getDistincElementsCounterMap(clasa);
		int maxim= Collections.max(counterMap.values());
		System.out.println(maxim); 
		String theClass="";
		for(String key: counterMap.keySet()) {
		    if(counterMap.get(key).equals(maxim)) {
