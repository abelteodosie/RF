...
public class FileUtils {
	...
	protected static void writeLearningSetToFile(String fileName, double[][] normalizedSet) { // first create the output
																								// to be written
		...
		{ // for each column
			for (int p = 0; p < normalizedSet[n].length; p++) {
				...
				if (p < normalizedSet[n].length - 1) { // then add separator
	...
	protected static double[][] normalizeLearningSet(double[][] learningSet) {
		...
		double[] jmin = new double[learningSet[0].length];
		double[] jmax = new double[learningSet[0].length];
		double max = -20000;
		double min = 20000;

		for (int n = 0; n < learningSet[0].length; n++) // for each column
		{ // for each row
			max = -20000;
			for (int p = 0; p < learningSet.length; p++) {
				if (learningSet[p][n] > max)
					max = learningSet[p][n];
		...
		{ // for each column
			for (int p = 0; p < learningSet[n].length; p++) {
				...
				if (jmax[p] - jmin[p] == 0)
					normalizedLearningSet[n][p] = 100;
				else
					normalizedLearningSet[n][p] = (learningSet[n][p] - jmin[p]) / (jmax[p] - jmin[p]);
			}
